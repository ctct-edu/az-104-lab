...
+ タスク 1: Azure portal と Azure Resource Manager テンプレートを使用して、ゾーン単位で冗長化された Azure 仮想マシンをデプロイする
...
+ タスク 5: Azure portal を使用してゾーン単位で冗長化されたAzure 仮想マシン スケール セットをデプロイする
...
1. Azure portal で、ポータルメニューから**「リソースの作成」**、**「Windows Server 2019 Datacenter」**を選択します。
...
1. **「次: 管理 >」** で、**「仮想マシンの作成」** ブレードの **「管理」** タブで、次の設定を指定します (他は既定値のままにします)。
    ...
    >**注**: 診断ストレージアカウントを新規に作成する際、名称が一意になる必要があります。適宜"XXXX"の部分を変更して（アカウント名の数字等）作成してください。例：az10408rg01diag0123

1. **「次: 詳細>」** をクリックし、**「仮想マシンの作成」** ブレードの **「詳細」** タブでは設定変更をせずに内容のみを確認し、**「確認および作成」** をクリックします。
...
1. 進行中のデプロイを表すテンプレートを確認し、**「デプロイ」** をクリックします。
    ...
    >**注**: このオプションを使用して、可用性ゾーン以外の項目が同一の構成となる 2 つ目の仮想マシンをデプロイします。
...
1. **「拡張機能のインストール」** ブレードに戻り、**「確認と作成」** をクリックし、検証後に **「作成」** をクリックします。
...
1. **「テンプレートの編集」** ブレードのテンプレートの内容を表示しているセクションで、**20** 行目から次のコードを挿入します ( ' '"resources": [ ' の次の行):
...
1. **「保存」** をクリックし、**「カスタム テンプレート」** ブレードの **「確認と作成」** をクリックし、**「確認と作成」** ブレードで **「作成」** をクリックします。
...
1. **「テンプレートの編集」** ブレードの 「テンプレートの内容を表示する」 セクションで、行 **51** `"dataDisks": []` ) を次のコードに置き換えます。
