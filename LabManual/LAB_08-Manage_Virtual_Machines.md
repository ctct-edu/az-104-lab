...
1. **「次へ: 管理 >」** で、**「仮想マシンの作成」** ブレードの **「ネットワーク」** タブで、次の設定を指定します (他は既定値のままにします)。
    ...
    >**注**: 診断ストレージアカウントを新規に作成する際、名称が一意になる必要があります。適宜"XXXX"の部分を変更して（アカウント名の数字等）作成してください。例：az10408rg01diag0727

1. **「次へ: 詳細>」** をクリックし、**「仮想マシンの作成」** ブレードの **「詳細」** タブで、利用可能な設定を変更せずに確認し、**「確認と作成」** をクリックします。
...
1. **「コマンド スクリプトの実行」** ブレードで、次のように入力し、**「実行」** をクリックして、単純なレイアウトと固定プロビジョニングを備えた 2 つのディスクで構成されるドライブ Z を作成します。
...
1. **「仮想マシン スケール セットの作成」** ブレードの **「詳細」** タブで、次の設定を指定し (他の設定は既定値のままにします)、 **「確認と作成」** をクリックします。
